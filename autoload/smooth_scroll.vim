...
function! smooth_scroll#line_down()
   if line(".") == line("$")
      return
   endif
   let diff = (line("w$") - line("."))
   if (line("$") - line(".")) <= 3
      exec "silent normal! j"
   else
      if diff <= 3
	 call s:smooth_scroll("d", 4, 0)
      else
	 exec "silent normal! j"
      endif
   endif
...
function! smooth_scroll#line_up()
   if line(".") == 1
      return
   endif

   let diff = (line(".") - line("w0"))
   if line("w0") <= 3
      exec "silent normal! k"
   else
      if diff <= 3
	 call s:smooth_scroll("u", 4, 0)
      else
	 exec "silent normal! k"
      endif
   endif
...
function! s:smooth_scroll(dir, dist, move)
  ...
  while l:current_line != l:target_line
    ...
    if l:scroll_frame_duration > l:elapsed*1.9 && l:scroll_lines_per_draw > 1
      " We can go faster
      let l:scroll_lines_per_draw = max([1, l:scroll_lines_per_draw-1])
    elseif l:scroll_frame_duration < l:elapsed*0.4
